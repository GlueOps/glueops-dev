...
|---------|--------------|-----------|
| `CAPTAIN_DOMAIN` | Inside code fences (` ``` `) and inline code spans (`` ` ``) | Swizzled `CodeBlock` (fences) / `CaptainDomainCode` (inline) — replace at render time |
| `CAPTAIN_NAMESPACE` | Inside code fences or inline code, where text needs the reader's environment namespace (first label of the captain domain, e.g. `nonprod`) | Swizzled `CodeBlock` (fences) / `CaptainDomainCode` (inline) — replace at render time |
...
1. **Code fences and inline code** — write `CAPTAIN_DOMAIN` as a literal sentinel. It is replaced with the reader's domain in fenced blocks and in inline backticks alike, so tables, prose and admonitions work without JSX components or an `.mdx` extension. Use `CAPTAIN_NAMESPACE` the same way for the environment namespace (the first label of the captain domain, e.g. `kubectl get pods -n CAPTAIN_NAMESPACE`).
   Because inline code is replaced, never put the raw sentinel in backticks when a sentence is *about* the sentinel — it renders as a domain. Rephrase instead.
