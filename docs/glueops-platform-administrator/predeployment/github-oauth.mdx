...
Assuming your **captain domain** is <CaptainDomain />, your application name should be: <code>dex.<CaptainDomain /></code>. 
...
1. **Homepage URL**: Specify the homepage URL for your OAuth app. The homepage URL contains the following
    ...
    - The application name. In this case <code>dex.<CaptainDomain /></code>
...
For the example cluster, the home page URL is <CaptainDomainLink to="https://dex.{domain}" />.

1. **Authorization callback URL**: Set the authorization callback URL for your OAuth app. Add `/callback` to the homepage URL. For the example cluster your authorization callback URL would be <CaptainDomainLink to="https://dex.{domain}/callback" />.
